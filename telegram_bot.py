...
def send_startup_message(paper_mode: bool, market_count: int) -> bool:
    ...
    message = (
        ...
        f"{'='*30}\n"
        f"\n"
        f"Mode: {mode} (SNIPER v2)\n"
        f"Monitoring: {market_count} weather markets\n"
        f"Threshold: {config.WHALE_THRESHOLD_MULTIPLIER}x average\n"
        f"Min whale size: ${config.WHALE_MIN_DOLLAR_SIZE:.0f}\n"
        f"Price range: {config.MIN_COPY_PRICE_CENTS}c-{config.MAX_COPY_PRICE_CENTS}c\n"
        f"Min confidence: {config.MIN_CONFIDENCE_SCORE}/100\n"
        f"Trade sizing: {'15% of portfolio per trade (REAL MONEY)' if not paper_mode else f'${config.PAPER_COPY_AMOUNT_CENTS/100:.0f} per trade (paper)'}\n"
        f"Poll interval: {config.POLL_INTERVAL_SECONDS}s\n"
        f"\n"
        f"Filters active:\n"
        f"  - Whale floor: ${config.WHALE_MIN_DOLLAR_SIZE:.0f}+ (real money only)\n"
        f"  - Price: {config.MIN_COPY_PRICE_CENTS}c-{config.MAX_COPY_PRICE_CENTS}c (good R/R only)\n"
        f"  - Confidence: {config.MIN_CONFIDENCE_SCORE}+ (quality signals only)\n"
        f"\n"
        f"Reports:\n"
        f"  - Morning briefing at 8 AM\n"
        f"  - End-of-day recap at 8 PM\n"
        f"\n"
        f"Commands: /status /today /recap /recap yesterday /balance /help\n"
        f"\n"
        f"Fewer trades. Better trades. I shall be watching.\n"
        f"Do try to get some sleep, Master Bruce."
    )
    return _send_message(message)
